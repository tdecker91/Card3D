...
func insert_card(card: Card3D, index: int):
	...
	card.card_3d_mouse_up.connect(_on_card_released.bind(card))
	card.card_3d_clicked.connect(_on_card_clicked.bind(card))
...
func remove_card(index: int) -> Card3D:
	...
	removed_card.card_3d_mouse_up.disconnect(_on_card_released.bind(removed_card))
	removed_card.card_3d_clicked.disconnect(_on_card_clicked.bind(removed_card))
...
func remove_all() -> Array[Card3D]:
	...
	for c in cards_to_return:
		...
		c.card_3d_mouse_up.disconnect(_on_card_released.bind(c))
		c.card_3d_clicked.disconnect(_on_card_clicked.bind(c))
...
func _on_card_released(card: Card3D):
